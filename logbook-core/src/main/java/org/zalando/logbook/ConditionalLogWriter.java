...
public final class ConditionalLogWriter implements HttpLogWriter {
    ...
    private final Predicate<? super HttpResponse> predicate;
    private final HttpLogWriter delegate;

    public ConditionalLogWriter(final Predicate<? super HttpResponse> predicate, final HttpLogWriter delegate) {
    ...
    public void writeResponse(final Correlation<String, String> correlation) throws IOException {
        if (predicate.test(correlation.getOriginalResponse())) {
